...
def parse_date_local(text: str) -> Optional[datetime]:
    ...
    # Önce ön işle
    preprocessed = _preprocess_turkish(text)

    # Özel tarihler
    special = _resolve_special_dates(preprocessed, now)
    if special:
...
def _adjust_for_weekend_holiday(dt: datetime) -> datetime:
    ...
    for _ in range(7):
        ...
        dt += timedelta(days=1)
        logger.info(f"Tatil/hafta sonu, ileri atıldı: {dt.strftime('%Y-%m-%d %A')}")
...
def parse_amount(text: str) -> Optional[float]:
    ...
    # "15.000 TL", "5000 lira", "15.500₺" vb.
    m = re.search(r"(\d[\d\.]*\d|\d)\s*(tl|lira|₺)", t)
    if m:
        # Binlik ayracı noktayı temizle
        raw = m.group(1)
        # 15.000 mu, 1.5 mu?
        if "." in raw:
            parts = raw.split(".")
            # Son parça 3 haneliyse → binlik ayracı (15.000)
...
def _fallback_structure(text: str) -> dict:
    ...
    # Müşteri adı: ilk büyük harfli kelime grubu
    customer = "Bilinmiyor"
    words = text.split()
    for i, w in enumerate(words):
        if w and w[0].isupper() and len(w) > 2:
            # Sonraki kelimeyi de ekle (varsa)
    ...
    # Tarih ipucu var mı? (basit keyword)
    ...
    return {
        ...
        "notes": "[Fallback parser - Ollama erişilemedi]",
    }
...
def parse_voice_text(text: str) -> dict:
    ...
    # 1. Tarih → dateparser (Türkçe)
    parsed_date = parse_date_local(text)
    logger.info(f"dateparser sonuç: {parsed_date}")

    # 2. Yapı → Ollama
    structure = _extract_structure(text)
    logger.info(f"Yapı: {structure}")
    ...
    # Tarih bulunamadıysa ama metinde de ipucu yoksa → eksik bilgi sor
    if not parsed_date and not has_clue:
        return {
            ...
            "needs_clarification": {
                "missing": "date",
                "question": f"{structure.get('customer_name', 'Bu müşteriyi')} ile ilgili ne zaman hatırlatayım? (örn: 'yarın', 'Cuma sabah', '25 Mayıs')",
            },
            "is_past_log": False,
        }

    # Tarih ipucu var ama dateparser çözemedi → hata
    if not parsed_date and has_clue:
        raise ValueError(
            "Tarihi anlayamadım. Daha açık söyle (örn: 'yarın saat 10', '25 Mayıs', 'gelecek Cuma')."
        )

    # 5. Saat default kontrolü
    due_date = parsed_date
    # Eğer dateparser saat döndürmediyse (00:00 = ipucusuz) → default saat
    if due_date.hour == 0 and due_date.minute == 0 and "saat" not in text.lower():
        # Sabah default 09:00 olarak göster ama kullanıcı saat söylemediyse 23:59 koy ki "X tarihinde" yorumu kalsın
        # Aslında biz saat çıkarmadık demektir, hatırlatma için sabah 9'u sonra hesaplayacağız
        pass  # 00:00 olarak bırak, aşağıda remind_at'i sabah 9'a çekeceğiz

    # 6. Geçmiş tarih kontrolü (log mode)
    now = datetime.now(tz)
    is_past_log = False
    if due_date < now:
        # Eğer aynı gün ama saat geçmişse → ileri at
        if (now - due_date) < timedelta(hours=12):
            # Aynı gün için ileri sabaha al
            due_date = (now + timedelta(hours=1))
        else:
            # Gerçekten geçmiş → log moduna al
            is_past_log = True
            logger.info(f"Geçmiş tarih, log mode: {due_date}")

    # 7. Hatırlatma zamanı
    if is_past_log:
        remind_at = now  # Yine de bir değer olsun
    else:
        # Saat 00:00 ise default hatırlatma 1 gün önce sabah 9
        if due_date.hour == 0 and due_date.minute == 0:
            ...
            # Saat belli → 1 gün önce aynı saatte değil, vade saatinden DEFAULT_REMINDER_HOURS_BEFORE önce
            remind_at = due_date - timedelta(hours=12)  # Yarım gün önce
            # Sabah saatlerine ayarla
            if remind_at.hour < 7:
