"""
Apple Reminders entegrasyonu - v2.1
...
def list_all_reminders() -> List[Dict]:
    """v2.1: İki yönlü senkron için tüm öğeleri çek"""
    backend = get_backend()
    if backend == "cli":
...
def get_two_way_sync_state() -> Dict[str, Dict]:
    """
    v2.2: Apple'daki tüm öğelerin durumunu döndür: {apple_id: {completed, title, due_date}}
...
def update_reminder_date(reminder_id: str, new_due: datetime) -> bool:
    """v2.2: Apple Reminders'taki tarihi güncelle (bot tarafından erteleme)"""
    if not reminder_id:
        return False
    backend = get_backend()
