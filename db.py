...
def init_db():
    ...
    with get_conn() as conn:
        ...
        # v2.2: User corrections (parser eğitimi için)
        conn.execute("""
...
def clone_reminder(source_id: int, new_due_date: datetime, new_remind_at: datetime) -> Optional[int]:
    """Aynısını tekrarla - kaynak hatırlatmayı kopyala"""
...
def get_smart_warnings(customer_name: str) -> List[str]:
    ...
    if created >= 3:
        ...
        if rate >= 40:
            warnings.append(
                f"⚠️ Bu musteri son {created} taahhudun {delayed}'ini geciktirdi (%{rate:.0f})"
            )

    if summary["open_amount"] and summary["open_amount"] > 50000:
        warnings.append(f"💸 Bu musteride zaten {summary['open_amount']:,.0f}TL acik alacak var")
    ...
    if summary.get("last_activity"):
        ...
        if days > 90:
            warnings.append(f"📅 Bu musteriyle son aktivitenin uzerinden {days} gun gecmis")

    if summary.get("personal_note"):
        warnings.append(f"📝 Musteri notu: {summary['personal_note']}")
