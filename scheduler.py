...
5. Akşam 19:00: kapanış raporu
6. Her gece 03:00: ses arşivi temizliği (30 günden eskileri sil)
7. Her 10 dk: bekleyen konuşmaları temizle (30 dk timeout)
8. Pazar gecesi 04:00: voice_ledger temizliği (7 günden eskileri sil)

Async-friendly: Tüm engelleme potansiyeli olan Apple Reminders ve disk I/O işlemleri asyncio.to_thread ile koşturulur.
"""
import logging
import asyncio
...
class ReminderScheduler:
    ...
    def start(self, authorized_chat_ids: list):
        ...
        # 2. Apple Reminders senkronu (her 5 dk)
        self.scheduler.add_job(
        ...
        # 3. Tekrarlayan materialize (her gece 00:05)
        self.scheduler.add_job(
        ...
        # 8. voice_ledger temizliği (haftalık Pazar 04:00)
        self.scheduler.add_job(
    ...
    async def check_due_reminders(self):
        """Eskalasyonlu hatırlatma gönderimi"""
        now = datetime.now(tz)
    ...
    async def sync_apple_reminders(self):
        """İki yönlü senkron + tarih senkronu (Asenkron wrapped)"""
        is_avail = await asyncio.to_thread(apple_reminders.is_available)
        if not is_avail:
            return
        try:
            apple_state = await asyncio.to_thread(apple_reminders.get_two_way_sync_state)
            if not apple_state:
                return
    ...
    async def materialize_recurring(self):
        """Asenkron wrapped"""
        try:
            count = await asyncio.to_thread(recurring.materialize_due_recurring)
            if count > 0:
    ...
    async def cleanup_audio_archive(self):
        """30 günden eski ses dosyalarını sil (I/O işlemi thread'e atılır)"""
        def _cleanup():
        ...
        try:
            ...
            if deleted_count > 0:
                logger.info(f"{deleted_count} eski ses dosyası temizlendi")
        except Exception as e:
