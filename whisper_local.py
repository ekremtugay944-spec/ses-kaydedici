...
def get_model() -> WhisperModel:
    global _model
    if _model is None:
        logger.info(f"Whisper modeli yukleniyor: {WHISPER_MODEL}")
        _model = WhisperModel(
            WHISPER_MODEL,
            device=WHISPER_DEVICE,
            compute_type=WHISPER_COMPUTE_TYPE,
        )
        logger.info("Whisper hazir")
    return _model
...
def transcribe_audio(audio_path: Path) -> str:
    ...
    """
    model = get_model()
    logger.info(f"Cevriliyor: {audio_path.name}")
    ...
    logger.info(f"Tamamlandi: {info.duration:.1f}s ses → {len(full_text)} karakter")
    return full_text
